...
#define UNBALANCED -1
int bst_check_height(node_t *node) {
  if (node == NULL)
    return 0;

  int left_h = bst_check_height(node->left);
  if (left_h == -1)
    return UNBALANCED;
  int right_h = bst_check_height(node->right);
  if (right_h == -1)
    return UNBALANCED;

  int diff_h = abs(left_h - right_h);
  if (diff_h > 1)
    return UNBALANCED;
...
int main(void) {
  ...
  if (len == 0) {
    ...
    // Un albero vuoto è un albero binario di ricerca e AVL
    printf("2\n");
    return 0;
  }
  ...
  int is_avl = bst_check_height(root) != UNBALANCED;
  if (!is_bst)
    printf("0\n");
  else if (!is_avl)
    printf("1\n");
